    ...
    protected boolean validate(HttpServletRequest request) {
        ...
        if (DataValidator.isNull(request.getParameter("address"))) {
            request.setAttribute("address",
                    PropertyReader.getValue("error.require", "College Address"));
            pass = false;
        }

        if (DataValidator.isNull(request.getParameter("state"))) {
            request.setAttribute("state",
                    PropertyReader.getValue("error.require", "College State"));
            pass = false;
        }
        if (DataValidator.isNull(request.getParameter("city"))) {
            request.setAttribute("city",
                    PropertyReader.getValue("error.require", "College City"));
            pass = false;
        }
        if (DataValidator.isNull(request.getParameter("phoneNo"))) {
            request.setAttribute("phoneNo",
                    PropertyReader.getValue("error.require", "College Phone Number"));
            pass = false;
        }else if (!DataValidator.isLong(request.getParameter("phoneNo"))) {
			request.setAttribute("phoneNo", PropertyReader.getValue("error.integer", "Phone Number"));
			pass = false;
		}else if ((request.getParameter("phoneNo").length()>10)||(request.getParameter("phoneNo").length()<10)) {
			request.setAttribute("phoneNo", "Phone Number should be of 10 digits");
			pass = false;
		}
